...
if col1.button("Run") or state["data_received"]:
    ...
    if not state["data_received"]:
        response = requests.post(
            "http://ec2-35-181-155-27.eu-west-3.compute.amazonaws.com:5000/predict_client", json={"SK_ID_CURR": int(sk_id_curr)}
        )
    ...
    fig_positive = generate_figure(
        top_positive_shap,
        "Les 10 prémières features augmentant le risque de non-remboursement",
        "right",
        "total ascending",
        "left",
    )
    fig_negative = generate_figure(
        top_negative_shap,
        "Les 10 prémières features réduisant le risque de non-remboursement",
        "left",
        "total descending",
        "right",
    )
